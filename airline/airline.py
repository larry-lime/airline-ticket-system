...
from airline.util import get_tickets, get_user_info, get_flight, get_purchases, ba_get_purchases
...
def index():
    ...
    if g.user_type == 'booking_agent':
        purchases = ba_get_purchases(g.user["username"], g.user["booking_agent_id"]) if user_is_logged_in() else None       
...
def purchase_ticket(id):
    ...
    if request.method == "POST":
        ...
        if g.user_type == 'customer':
            ...
            query = """
                    ...
                    VALUES ({},'{}','{}','{}')
                    """
            cursor.execute(query.format(ticket_id, customer_email, purchase_date, booking_agent_id))
            conn.commit()
            cursor.close()
    ...
    return render_template(
        "airline/purchase_ticket.html", flight=flight, tickets=tickets,customers=customers
    )
...
def staff_action():
    ...
    if request.method == "POST":
        ...
        elif (permission_type['permission_type'] == "write" or\
            permission_type['permission_type'] == "admin") and\
            staff_action == "change_status":
        ...
        elif permission_type['permission_type'] == "admin" and staff_action == 'grant_new_permission':
    ...
    if request.method == 'POST':
        ...
        if flight_data is not None:
            ...
            query = """
                ...
                '{}','{}','{}') 
                """
            cursor.execute(query.format(flight_num, airline_name, departure_airport,\
                departure_time, arrival_airport, arrival_time,
                price, STATUS, airplane_id))
            conn.commit()
            cursor.close()
    ...
    if request.method == 'POST':
        ...
        if data is not None:
            ...
            query = """
                ...
                VALUES ('{}','{}','{}','{}','{}') 
                """
            cursor.execute(query.format(username, password,first_name,last_name,booking_agent_id))
            conn.commit()
            cursor.close()
            return redirect(url_for('airline.index'))
